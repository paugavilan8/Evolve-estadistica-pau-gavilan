...
def preprocesar(df: pd.DataFrame):
    ...
    - Elimina columnas que no aportan información al modelo o causan leakage.
    ...
    - 'Name' y 'Developer' tienen demasiadas categorías únicas → no aportan
      capacidad predictiva generalizable y generan ruido.
    ...
    - 'NA_Sales', 'EU_Sales', 'JP_Sales', 'Other_Sales' son componentes
      directas de Global_Sales (Global_Sales = suma de todas ellas) → incluirlas
      causaría data leakage y un R²≈1 artificial. Se eliminan obligatoriamente.
    ...
    # Columnas a eliminar:
    # - Alta cardinalidad o no útiles: Name, Developer, Publisher
    # - Muchos nulos: User_Score, Critic_Score, Critic_Count, User_Count
    # - DATA LEAKAGE: NA_Sales, EU_Sales, JP_Sales, Other_Sales son
    #   componentes directas de Global_Sales → se eliminan obligatoriamente
    cols_eliminar = [
        'Name', 'Developer', 'User_Score',
        'Critic_Score', 'Critic_Count', 'User_Count',
        'NA_Sales', 'EU_Sales', 'JP_Sales', 'Other_Sales'  # anti-leakage
    ]
...
def entrenar_regresion_lineal(X_train, y_train) -> LinearRegression:
    ...
    Parámetros:
        ...
        y_train: Target de entrenamiento (array-like).
...
def evaluar_modelo(modelo: LinearRegression, X_test, y_test,
    ...
    Muestra también las 10 variables más influyentes por valor absoluto
    del coeficiente.
 
    Parámetros:
        modelo (LinearRegression): Modelo entrenado.
        X_test: Features del test set.
        y_test: Target real del test set.
        feature_names (list): Nombres de las features.
 
    Retorna:
        dict: Diccionario con MAE, RMSE, R², y_pred e y_test.
    """
...
def guardar_metricas(metricas: dict) -> None:
    """
    Guarda las métricas del modelo en un fichero de texto.
 
    Parámetros:
        metricas (dict): Diccionario con MAE, RMSE y R².
    ...
    with open(out_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"\nNota: Se han excluido NA_Sales, EU_Sales, JP_Sales y Other_Sales\n")
        f.write(f"para evitar data leakage (Global_Sales = suma de ventas regionales).\n")
...
def plot_residuos(metricas: dict) -> None:
    """
    Genera y guarda el gráfico de residuos del modelo de regresión lineal
    (valores predichos en X, residuos en Y).
 
    Parámetros:
        metricas (dict): Diccionario con y_pred e y_test.
